...
애널리스트가 작성한 SEC 문서 분석 보고서를 **CEO에게 보고하기 전에** 품질을 평가합니다.
...
3. **원문 인용 (Direct Quotes)**: 영어 원문이 직접 인용되어 있는가?
   - 0점: 원문 인용 없음
   - 1점: 1-2개 인용
   - 2점: 3개 이상 중요 원문 인용
...
class AnswerRefiner:
    ...
    REFINEMENT_PROMPT = """당신은 SEC 문서 분석 전문가입니다.
...
4. **원문 인용 추가**: 핵심 문장을 영어 원문으로 직접 인용
...
class IterativeAnswerer:
    ...
    INITIAL_PROMPT = """당신은 SEC 문서를 분석하는 **시니어 투자 리서치 애널리스트**입니다.
...
- **영어 원문 인용**: 핵심 문장은 "영어 원문" 형태로 직접 인용
- **출처 표기**: 모든 사실에 [Form YYYY-MM-DD | Page X | Section] 형식
...
- 모든 수치에 출처 표기"""
    ...
    def generate(
        ...
        messages = [
            ...
            HumanMessage(content=f"""## SEC 문서 컨텍스트
